...
## Some renaming for interpretability:
## Mapping:
...
comp.class.t1 <- distCompare(pairs.class, theta="CultureThreat",
                             var="class")

comp.class.t2 <- distCompare(pairs.class, theta="ReligionThreat",
                             var="class")
comp.class.t3  <- distCompare(pairs.class,
                              theta="EconThreat", var="class")
...
table(comp.all$label) ## 2374 no; 99 yes 

comp.all[which(comp.all$label==1), "name"]
...
## Plot-- Not Actually Useful to Visualize
## Too many overlaps
gg.c <- ggplot(data=comp.all,
    ...
    ggtitle("Kolmogorov Smirnov Test Results",
            subtitle="Paiwise Within Class, Media Trust, Political Orientation, Country Variables")+
    facet_wrap(~PresVar, ncol=1)+
    theme_bw()
    
gg.c

ggsave(gg.c,
       file="kstestgraph.pdf")               
...
cor(thetas$CultureThreat, thetas$SupportIm)
cor(thetas$EconThreat, thetas$SupportIm)

colnames(thetas)

corvars <- c("CultureThreat",
             "ReligionThreat",
             "EconThreat",
             "HealthThreat",
             "SupportIm",
